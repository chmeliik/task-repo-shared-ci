...
Checks versioning requirements for the files that got modified/added between the
base revision (defaults to `main`) and your current HEAD. Run the script with
...
> The script simply marks the highest found version as the one that started tracking
> changes in CHANGELOG.md. If the highest found version is `<=0.1.0`, it instead marks
> this as the initial version of the Task.
