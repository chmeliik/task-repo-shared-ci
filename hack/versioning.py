...
class TaskFile:
    ...
    def read_at_base_ref(self, base_ref: str) -> TaskContent:
        proc = subprocess.run(
            ["git", "show", f"{base_ref}:{self.path}"],
            capture_output=True,
            text=True,
            check=True,
        )
        return TaskContent(proc.stdout)
...
# --- Git changeset + listing task files ---
...
def check(base_ref: str = "main", paths: list[Path] | None = None) -> Iterator[Result]:
    ...
    for task_file in task_files:
        ...
        if not changelog_exists:
            yield Result(result_kind, f"CHANGELOG.md missing at {changelog_path}", task_file.path)
        ...
        if status == "modified":
            ...
            if changelog_exists and not changeset.did_change(changelog_path):
                yield Result(
                    "warning", f"CHANGELOG.md at {changelog_path} is unchanged", task_file.path
                )
    ...
    for task_dir in sorted(task_dirs):
        ...
        for task_file in list_task_files([task_dir]):
            ...
            try:
                ...
                yield Result(
                    "error",
                    f"{e}. Cannot determine maximum version for {task_dir}.",
                    task_file.path,
                    task_content.version_line,
                )
...
def make_parser() -> argparse.ArgumentParser:
    ...
    check_parser.add_argument(
        "--base-ref", default="main", help="Base git ref (default: main)"
    )
    check_parser.add_argument(
